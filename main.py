...
# Функція для переписування вмісту CSV у JSON з додаванням нових рядків
def csv_in_json_data():
    try:
        # Читання даних з CSV файлу
        data = []
        with open('countries_updated.csv', mode='r') as csv_file:
            csv_reader = csv.DictReader(csv_file)
            for row in csv_reader:
                # Конвертуємо значення area і population у відповідні типи даних
        ...
        # Додаємо нові країни
        new_data = [
            {"country": "Іспанія", "area": 506, "population": 48},
            {"country": "Італія", "area": 302.1, "population": 60},
            {"country": "Польща", "area": 322.6, "population": 37}
        ]
        data.extend(new_data)

        # Запис у JSON файл
        with open('countries_new_updated.json', mode='w', encoding='utf-8') as json_file:
            json.dump(data, json_file, indent=4, ensure_ascii=False)

        print("JSON файл 'countries_final.json' успішно створено.")

    except FileNotFoundError:
        print("Помилка: Файл 'countries_updated.csv' не знайдено.")
    except json.JSONDecodeError:
        print("Помилка: Не вдалося декодувати JSON.")
    except Exception as e:
        print(f"Невідома помилка: {e}")
